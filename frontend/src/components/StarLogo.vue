...
<template>
  <div class="star-logo" :class="{ active }" role="img" aria-label="College Compass">
    <svg viewBox="0 0 120 120" class="orbit">
      <circle cx="60" cy="60" r="7" fill="var(--gold-leaf)" class="center-star" />
      <g class="ring">
        <circle cx="60" cy="16" r="4" fill="var(--reach-ember)" />
      </g>
      <g class="ring ring-reverse">
        <circle cx="60" cy="30" r="3.5" fill="var(--target-sage)" />
      </g>
      <g class="ring">
        <circle cx="60" cy="44" r="3" fill="var(--likely-teal)" />
      </g>
    </svg>
  </div>
</template>
...
.star-logo {
  ...
  transition: top 0.7s cubic-bezier(0.65, 0, 0.35, 1), transform 0.7s cubic-bezier(0.65, 0, 0.35, 1);
  z-index: 20;
  pointer-events: none;
}
.star-logo.active {
